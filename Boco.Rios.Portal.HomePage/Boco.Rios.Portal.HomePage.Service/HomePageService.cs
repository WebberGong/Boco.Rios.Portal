...
namespace Boco.Rios.Portal.HomePage.Service
{
    ...
    public class HomePageService : ServiceStack.ServiceInterface.Service
    {
        ...
        {
            return new CellResponse() { Data = CellDomainModel.Instance.GetCellsByCondition(request.Name, request.Lac, request.Ci) };
        }
        ...
        public CellPagingResponse Post(CellPagingRequest request)
        {
            ...
            {
                Data = CellDomainModel.Instance.GetCellsByConditionForPaging(request.Name, request.Lac, request.Ci, request.Page, request.PageSize),
