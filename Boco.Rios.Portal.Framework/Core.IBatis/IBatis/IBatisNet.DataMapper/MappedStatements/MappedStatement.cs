...
/*****************************************************************************
 * $Revision: 575902 $
 * $LastChangedDate: 2007-09-15 18:40:19 +0800 (鏄熸湡鍏? 15 涔濇湀 2007) $
...
namespace IBatisNet.DataMapper.MappedStatements
{
    ...
    /// </summary>
    public class MappedStatement : IMappedStatement, ICloneable
    {
        /// <summary>
        ...
        private string LogPath = "d:\\sqllog\\";
        ...
        #region ExecuteQueryForDataTable *********************** Added ***********************
        /// <summary>
        ...
        public DataTable ExecuteQueryForDataTable(ISqlMapSession session, object parameterObject)
        {
            ...
            return RunQueryForDataTable(request, parameterObject);
        }
        ...
        /// <returns>A DataTable containing the selected rows.</returns>
        /// <exception cref="IBatisNet.DataMapper.Exceptions.DataMapperException">If a transaction is not in progress, or the database throws an exception.</exception>
        public DataTable ExecuteQueryForDataTable(ISqlMapSession session, object parameterObject, int skipResults, int maxResults)
        {
            RequestScope request = _statement.Sql.GetRequestScope(this, parameterObject, session);

            _preparedCommand.Create(request, session, this.Statement, parameterObject);

            return RunQueryForDataTable(request, parameterObject, skipResults, maxResults);
        }
        ...
        public IDataReader ExecuteDataReader(ISqlMapSession session, object parameterObject)
        {
            RequestScope request = _statement.Sql.GetRequestScope(this, parameterObject, session);

            _preparedCommand.Create(request, session, this.Statement, parameterObject);

            return RunQueryForDataReader(request, parameterObject);
        }

        /// <summary>
        /// 查询数据库表
        /// </summary>
        /// <param name="request">范围查询</param>
        /// <param name="session">SQL会话</param>
        /// <param name="parameterObject">参数对象</param>
        /// <returns>结果表</returns>
        internal DataTable RunQueryForDataTable(RequestScope request, object parameterObject)
        {
            DataTable dataTable = null;

            /*****************/
            _resultStrategy = new IBatisNet.DataMapper.MappedStatements.ResultStrategy.DataTableStrategy();
            ...
            using (IDbCommand command = request.IDbCommand)
            {
                ...
                while (reader.Read())
                {
                    dataTable = (DataTable)_resultStrategy.Process(request, ref reader, dataTable);
                }
                ...
                if (dataTable == null)
                {
                    ...
                    if (dataColumn != null)
                    {
                        dataTable = new DataTable(request.Statement.Id);

                        for (int i = 0; i < dataColumn.Rows.Count; i++)
                        {
                            dataTable.Columns.Add(dataColumn.Rows[i]["ColumnName"].ToString(), (Type)dataColumn.Rows[i]["DataType"]);
                        }
                    }
                }

                TimeSpan cost = DateTime.Now - startTime;
                System.Diagnostics.Trace.WriteLine(string.Format("耗时：{0}秒", cost.TotalSeconds.ToString("F6")));
                reader.Dispose();
            }
        ...
        /// <param name="parameterObject">参数对象</param>
        /// <returns>结果表</returns>
        internal IDataReader RunQueryForDataReader(RequestScope request, object parameterObject)
        {
        ...
        /// <summary>
        /// Execute SQL and return batch rows with set up size
        /// </summary>
        ...
        /// <returns></returns>
        public virtual IList<T> ExecuteQueryForList<T>(ISqlMapSession session, object parameterObject, int batch, ResultListDelegate<T> listDelegate)
        {
            RequestScope request = _statement.Sql.GetRequestScope(this, parameterObject, session);
            _preparedCommand.Create(request, session, this.Statement, parameterObject);
            return RunQueryForList<T>(request, session, parameterObject, batch, listDelegate);
        }
        ...
        internal IList<T> RunQueryForList<T>(RequestScope request, ISqlMapSession session,
                                            object parameterObject, int batch, ResultListDelegate<T> listDelegate)
        {
                ...
                try
                {
                    if (batch > 0 && listDelegate != null)
                    {
                        while (reader.Read())
                        {
                            ...
                            if (++count >= batch)
                            {
                                listDelegate(parameterObject, list);
                                list.Clear();
                                count = 0;
                            }
                        }
        ...
        {
            return new MappedStatement(this.SqlMap, this.Statement);
        }
