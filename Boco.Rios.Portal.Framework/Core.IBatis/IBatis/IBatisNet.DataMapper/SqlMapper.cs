...
/*****************************************************************************
 * $Revision: 591621 $
 * $LastChangedDate: 2007-11-03 21:44:57 +0800 (鏄熸湡鍏? 03 鍗佷竴鏈?2007) $
...
namespace IBatisNet.DataMapper
{
	...
	public class SqlMapper : ISqlMapper
	{
        ...
        #region *********************** QueryForDatatable,SetConfig ***********************
        /// <summary>
        /// Executes a Sql SELECT statement that returns a datatable 
        /// </summary>
        ...
        /// Feature idea by gbayon, test by Mr.yuance
        /// </summary>
        /// <param name="statementName">The name of the sql statement to execute.</param>
        /// <param name="parameterObject">The object used to set the parameters in the SQL.</param>
        ...
        /// <returns>DataTable</returns>
        public DataTable QueryForDataTable(string statementName, object parameterObject, int skipResults, int maxResults)
        {
            bool isSessionLocal = false;
            ISqlMapSession session = _sessionStore.LocalSession;
            DataTable dataTable;
            ...
            {
                IMappedStatement statement = GetMappedStatement(statementName);
                dataTable = statement.ExecuteQueryForDataTable(session, parameterObject,skipResults,maxResults);
            }
            catch
            {
                throw;
        ...
        public IDataReader ExecuteDataReader(string statementName, object parameterObject)
        {
            //bool isSessionLocal = false;
            ISqlMapSession session = _sessionStore.LocalSession;
            ...
            {
                IMappedStatement statement = GetMappedStatement(statementName);
                dataReader = statement.ExecuteDataReader(session, parameterObject);
            }
            catch
            {
                throw;
        ...
        /// <summary>
        /// 设置数据库服务器环境变量，临时解决办法(建议以后全部采用存储过程)
        /// </summary>
        /// <param name="statementName"></param>
        /// <param name="parameterObject"></param>
        public void ExecuteSetConfig(string statementName, object parameterObject)
        {
            //bool isSessionLocal = false;
            ISqlMapSession session = _sessionStore.LocalSession;
            ...
            {
                IMappedStatement statement = GetMappedStatement(statementName);
                statement.ExecuteUpdate(session, parameterObject);
            }
            catch
            {
                throw;
        ...
        /// <summary>
        /// Execute SQL with sized batch result list
        /// </summary>
        ...
        /// <returns></returns>
        public IList<T> QueryForList<T>(string statementName, object parameterObject, int batch, ResultListDelegate<T> listDelegate)
        {
            bool isSessionLocal = false;
            ISqlMapSession session = _sessionStore.LocalSession;
            IList<T> list;
            ...
            {
                IMappedStatement statement = GetMappedStatement(statementName);
                list = statement.ExecuteQueryForList<T>(session, parameterObject, batch, listDelegate);
            }
            catch
            {
                throw;
        ...
        public int BulkCopyAutoTransaction(string targetTable, DataTable sourceData)
        {
            ...
            try
            {
                rows = session.BulkCopyAutoTransaction(targetTable, sourceData);
            }
            catch
            {
                throw;
        ...
        public int BulkCopyLocalTransaction(string targetTable, DataTable sourceData)
        {
            ...
            try
            {
                ...
                //    throw new DataMapperException("No local transaction started");

                rows = session.BulkCopyLocalTransaction(targetTable, sourceData);
            }
		...
		public IMappedStatement GetMappedStatement(string id) 
		{
            ...
            return (IMappedStatement)((ICloneable)_mappedStatements[id]).Clone();
		}
